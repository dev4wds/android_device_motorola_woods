...
public class WoodsRIL extends RIL implements CommandsInterface {
    ...
    public WoodsRIL(Context context, int preferredNetworkType, int cdmaSubscription) {
        super(context, preferredNetworkType, cdmaSubscription, null);
        //mContext = context;
        Rlog.i("WoodsRIL", "Ctor1: context is " + mContext);
        mTelephonyManager = (TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE);
        mEccList = new MtkEccList();
    }

    public WoodsRIL(Context context, int preferredNetworkType,
            int cdmaSubscription, Integer instanceId) {
        super(context, preferredNetworkType, cdmaSubscription, instanceId);
        //mContext = context;
        Rlog.i("WoodsRIL", "Ctor2: context is " + mContext);
        mTelephonyManager = (TelephonyManager) context.getSystemService(Context.TELEPHONY_SERVICE);
        mEccList = new MtkEccList();
    }
    ...
    iccIOForApp (int command, int fileid, String path, int p1, int p2, int p3,
        ...
        if (command == 0xc0 && p3 == 0) {
            Rlog.i("WoodsRIL", "Override the size for the COMMAND_GET_RESPONSE 0 => 15");
            p3 = 15;
        }
